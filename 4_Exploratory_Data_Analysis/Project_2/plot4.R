# setup directory
setwd("F:/Data Science/4. Exploratory Data Analysis/Week3/Project2")
getwd()

#load the datasets
file <- readRDS("Source_Classification_Code.rds")
sFile <- readRDS("summarySCC_PM25.rds")

colnames(file)
dim(file)

dim(sFile)
colnames(sFile)
unique(sFile$type)

# extract the SCC data from file dataset
coal <- file[grepl("Coal",file$EI.Sector),]$SCC
head(coal)

#subset sFile data set based on coal
subset <- sFile[sFile$SCC %in% coal, ]
dim(subset)

#group the data based on year
group <- tapply(sFile$Emissions, sFile$year, sum, na.rm = T)
group

#from the group data
totalEmissions <- c(7332967, 5635780, 5454703, 3464206)
year <- c(1999, 2002, 2005,2008)

#create data frame
DF <- data.frame(totalEmissions, year)
DF

#plot the group data
plot(totalEmissions ~ year, data = DF, type = "b", col = "purple", lwd = 2, pch = 16, 
        main = "Emissions from coal combustion-related sources",
        xlab = "Years",
        ylab = "Total Emission per year")

#save the plot as a png file
dev.copy(png, file = "plot4.png", width = 480, height = 480)
dev.off()

